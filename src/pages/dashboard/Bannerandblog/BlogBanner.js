...
const BlogBanner=()=>{
  ...
  const navigate = useNavigate();
    ...
    const tableHeaderData=[
      { field: 'img_url',headerName:"Picture",filterable:false, width: 130, renderCell: (params) => (
        ...
        { field: 'featured',headerName:"Featured", width: 200      },
      { field: 'youtube_video_id',headerName:"Youtube_Video Id", width: 190 },
...
//         ) },
        { field: 'visibility',headerName:"Item Visibility", width: 200
        ...
        { field: 'id' ,headerName:"Actions" ,width: 150,renderCell: (params) => (
            <Box sx={{display:"flex"}}>
               {/* <IconButton component={RouterLink}
                ...
                {(user === 'DM' || user === 'Admin') &&
               <IconButton component={RouterLink}
                to={`${PATH_DASHBOARD.BlogBanner.edit}/${params.value}`}> <EditIcon sx={{fontSize:"20px",marginRight:"4px"}}/></IconButton> }
                {/* {(user === 'DM' || user === 'Admin') &&
...
const getitemsList=React.useCallback(async()=>{
  try{
    const params={
     ...
     console.log(params)
    const res = await Item_Details.Getbannerblog(Token,params);
          ...
          <HeaderBreadcrumbs
            ...
            action={
              <Button
                ...
                to={(user === 'DM' || user === 'Admin') && PATH_DASHBOARD.BlogBanner.create}
                startIcon={
