...
const DairyCreate = () => {
  ...
  const UpdateUserSchema = Yup.object().shape({
    ...
    procurement_mgr: Yup.string().required("Procurement Manager Name is required"),
    ...
    // logo_url: Yup.mixed().required("Email is required"),
    // additional_info: Yup.mixed().required("additional_info is required"),
    address_line1: Yup.mixed().required("Address_line1 is required"),
    address_line2: Yup.mixed().required("Address_line1 is required"),
    // address_line3: Yup.mixed().required("Address_line1 is required"),
    ...
    onSubmit: async (values, { setErrors, setSubmitting }) => {
      try {
        ...
        const data = {
          ...
          procurement_mgr:values.procurement_mgr,
        ...
        console.log(JSON.stringify(data)+'jhjgjg')
        const res = await Item_Details.DairyCreate(data, Token);
...
useEffect(
 ...
 return (
    <Page title="Dairy Management | Animeta">
        ...
        <HeaderBreadcrumbs
          ...
          links={[
            { name: "Dashboard", href: PATH_DASHBOARD.root },
            {
              ...
              href: PATH_DASHBOARD.dairy.root,
            },
            {
              name: "Item Create",
            },
...
<TextField
                 ...
                 {...getFieldProps(`procurement_mgr`)}
                 error={Boolean(touched.procurement_mgr && errors.procurement_mgr)}
                 helperText={touched.procurement_mgr && errors.procurement_mgr}
               />
                           </Stack>
